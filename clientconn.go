...
func (ac *addrConn) resetTransport(resolveNow bool) {
	...
	addrLoop:
		for _, addr := range ac.addrs {
			...
			// If we've received a preface, the connection is fully READY. We'll
			// reconnect from the top with a fresh backoff. If not, the
			// connection only got to half-READY, so we'll continue from where
			// we left off as if this had been a failure.
			select {
			case <-prefaceReceived:
				ac.mu.Lock()
				// After a successful connection, the next reconnect should begin
				// without backoff.
				ac.backoffIdx = -1
				ac.mu.Unlock()
		...
		// After exhausting all addresses, or after a READY, the addrConn enters TRANSIENT_FAILURE.
		ac.mu.Lock()
		if ac.state == connectivity.Shutdown {
			ac.mu.Unlock()
			return
	...
	if err == nil {
		if ac.dopts.waitForHandshake {
			...
			case <-onCloseCalled:
				...
				return nil, errors.New("connected closed")
			}
		} else {
			go func() {
				select {
