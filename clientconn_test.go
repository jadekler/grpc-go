...
// tryUpdateAddrs should cause the next reconnect to begin from the top of the
// list.
func TestTryUpdateAddrs_RetryFromFirstAddr(t *testing.T) {
	...
	client.mu.Unlock()
	if ok := ac.tryUpdateAddrs(addrsList); !ok {
		t.Fatal("was not able to update addrs")
	}
